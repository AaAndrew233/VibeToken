...
> [!NOTE]
> 当前为早期预览版，已支持 22 个 AI 编程数据源。当前版本是 [v0.1.8 Preview](https://github.com/AaAndrew233/VibeToken/releases/tag/v0.1.8)。GitHub Releases 提供可下载安装包，但安装包尚未经过 Apple 公证。
...
- 可选连接 Sub2API；自动额度刷新保持只读，展示账号额度、重置时间、套餐到期时间和异常状态，并提供需要单独确认的手动 OAuth 凭证刷新来更新套餐信息。
- 原生 macOS 菜单栏与程序坞入口，支持英文和简体中文；程序坞图标和开机自动启动仅在独立“设置”窗口中管理，打开设置时菜单栏弹框保持显示。
...
- 当前版本：[v0.1.8 Preview](https://github.com/AaAndrew233/VibeToken/releases/tag/v0.1.8)。
...
2. 选择今天、24H、7D 或 30D。
3. 为本地用量采集选择实时、5 分钟、30 分钟或手动刷新。实时模式由文件变化触发，会合并连续写入，并以 60 秒兜底扫描处理可能漏掉的事件。连接后的 Sub2API 账号池每 30 秒检查账号变化，并且每 30 分钟执行一次带验真的正式额度刷新。
4. 点击菜单栏弹窗右上角的齿轮按钮打开“设置”，调整程序坞图标显示方式和开机自动启动；设置窗口出现时菜单栏弹框保持显示，这两个选项不再在弹框其他位置重复展示。

如需监控 Sub2API，在原有“中转额度”区域使用管理员账号登录。首次同步后，检测为 `Plus` 的账号固定按 `Plus`（1 倍）计算；检测为 `Pro` 的账号必须手动选择 `Pro 5x`、`Pro 10x` 或 `Pro 20x`。未配置的 Pro 账号不会套用猜测默认值，配置完成前不展示账号池额度。App 启动、Mac 唤醒、账号池变化以及每 30 分钟调度，都会以 `force=true` 请求 Sub2API 官方额度探测，但不会修改 OAuth 凭证。连接 Sub2API 后手动点击刷新时，VibeToken 会询问“仅刷新额度”还是“更新套餐与额度”；后者会明确调用 Sub2API 的批量 OAuth 凭证刷新，只处理 active 实体账号，可能轮换服务端保存的 Token，最长等待 120 秒，之后再执行正式额度刷新并回读账号列表。对于没有批量额度路由的正式版服务，会回退到 `source=active&force=true` 的单账号接口，并发最多 6 个请求。只有整池验真通过才发布总额度；套餐或额度部分刷新失败时只报告已验证账号数，不会误报全部成功。VibeToken 不会重置或删除中转账号。
...
- Sub2API 自动监控保持只读；只有用户明确确认“更新套餐与额度”后才会请求 Sub2API 刷新 OAuth 凭证，这可能轮换中转账号在服务端保存的 Access Token 和 Refresh Token。
